...
public class ProblemasPropuestos5 {
	...
	static void controlarriba(){
		...
		if(controlbarco){
			...
			if(opcion=='s'){for(i=0;i<tamaño;i++){hundir[f-i][c]=1;}controlposicion=false;}
	...
	static void controlderecha(){
		...
		if(controlbarco){
			...
			System.out.println("Desea esta opcion(s/n)");
			opcion=teclado.next().charAt(0);
			if(opcion=='s'){for(i=0;i<tamaño;i++){hundir[f][c+i]=1;}controlposicion=false;}
	...
	static void controlarribamaquina(){
		...
		if(controlbarco){
			for(i=0;i<tamaño;i++){hundir2[fm-i][cm]=1;}
	...
	static void controlizquierdamaquina(){
		...
		if(controlbarco){
			for(i=0;i<tamaño;i++){hundir2[fm][cm-i]=1;}
			controlposicion=false;
			cambiarmaquina();}
...
valores (2, 3, etc.) para diferentes barcos; así podremos saber si hemos hundido cada
barco o sólo lo hemos tocado. [100]: Agua ya bombardeada. [101]: Barco ya
...
coordenadas que quiere bombardear, mostrándole el tablero después de cada jugada.
